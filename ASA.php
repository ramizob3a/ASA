...
//@TH3VICTORY and contact me ( @TH3VICTORY )
//Developer @ASA
...
class ASA {
    ...
    public function api($method, $datas = []) {
        ...
        //$url = 'https://th3victory.tk/TelegramAPI/bot' . $this->token . '/' . $method;
        $ch = curl_init($url);
        ...
        $res = curl_exec($ch);
        if (curl_error($ch)) {
            CreateLogRami(curl_error($ch));
        } else {
            $decode = json_decode($res, 1);
            if ($decode['ok'] != true) {
                if ($decode['description'] == "Unauthorized") {
                    CreateLogRami("Error Connect API Telegram : Token is not correct");
                } else {
                    CreateLogRami("Error API Telegram : " . $decode['description']);
                }
            }
            return $res;
        }
    ...
    public function SendVideo($width = null, $height = null) {
        ...
        public function IsJoinUser($channel) {
            ...
            if (@$api['description'] == 'Bad Request: chat not found') {
                CreateLogRami("Channel Not found! Are You Sure $channel exists?");
                return "CNF1XR3";
            } elseif (@$api['description'] == 'Bad Request: CHAT_ADMIN_REQUIRED') {
                CreateLogRami("The Bot is Not Admin in the channel");
                return "BNA1XR3";
            } else {
...
"png_sticker"=>new CurlFile($sticker)]),1);
    ...
    class ASA_JsonData {
        ...
        public function SetNameFile($data) {
            if ($data == null) {
                CreateLogRami("Where Is SetNameFile!?");
            } else {
                $this->NameJson = $data;
            }
        }
    ...
    class ASA_Mysql {
        public function __construct($Server, $Username, $Password,$Name=null) {
            if (function_exists('mysqli_connect')) {
                ...
                if (!$mysql) {
                    CreateLogRami("DataBase Error : " . mysqli_connect_error());
                    return false;
                } else {
...
");
        ...
        public function CreateTable($CHARACTER="BINARY") {
            ...
            if ($name == null) {
                CreateLogRami("Can t Find Table Name in DBTable");
            }
...
Error : " . $this->mysql->error);
        ...
        public function TableExists() {
            ...
            if ($table == null) {
                CreateLogRami("Can t Find Table Name in DBTableExists");
            }
...
Error : " . $this->mysql->error);
        ...
        public function IDS($find='*') {
            $table = $this->TableName;
            if ($table == null) {
                CreateLogRami("Can t Find Table Name in IDS");
            }
            $array = [];
...
Error : virable value not integers");
        ...
        public function DelCustomProfile($row, $value) {
            $table = $this->TableName;
            if ($table == null) {
                CreateLogRami("Can t Find Table Name in DelCustomProfile");
            }
...
Error : " . $this->mysql->error);
        ...
        public function AddCustomProfile($row, $value) {
            $table = $this->TableName;
            if ($table == null) {
                CreateLogRami("Can t Find Table Name in AddCustomProfile");
            }
...
Error : " . $this->mysql->error);
        ...
        public function AddProfile($chat_id) {
            ...
            if ($chat_id == null) {
                CreateLogRami("Can t Find Chat ID in DBGet! Are you sure you can use the AutoChatID?");
            }
...
Error : " . $this->mysql->error);
        ...
        public function DelProfile($chat_id) {
            $table = $this->TableName;
            if ($table == null) {
                CreateLogRami("Can t Find Table Name in DelProfile");
            }
            if ($chat_id == null) {
                CreateLogRami("Can t Find Chat ID in DelProfile! Are you sure you can use the AutoChatID?");
            }
...
Error : " . $this->mysql->error);
        ...
        public function AddColumn($name, $definition) {
            $table = $this->TableName;
            if ($table == null) {
                CreateLogRami("Can t Find Table Name in AddColumn");
            }
...
Error : " . $this->mysql->error);
        ...
        public function DelColumn($name) {
            $table = $this->TableName;
            if ($table == null) {
                CreateLogRami("Can t Find Table Name in DelColumn");
            }
...
Error : " . $this->mysql->error);
        ...
        public function PutNoWhere($key, $value) {
            $table = $this->TableName;
            if ($table == null) {
                CreateLogRami("Can t Find Table Name in PutNoWhere");
            }
...
Error : " . $this->mysql->error);
        ...
        public function GetNoWhere($key, $chat_id) {
            $table = $this->TableName;
            if ($table == null) {
                CreateLogRami("Can t Find Table Name in GetNoWhere");
            }
            if ($chat_id == null) {
                CreateLogRami("Can t Find Chat ID in GetNoWhere! Are you sure you can use the $chat_id?");
            }
...
Error : " . $this->mysql->error);
        ...
        public function SUM($column) {
            $table = $this->TableName;
            if ($table == null) {
                CreateLogRami("Can t Find Table Name in SUM");
            }
        ...
        public function COUNT($column) {
            $table = $this->TableName;
            if ($table == null) {
                CreateLogRami("Can t Find Table Name in COUNT");
            }
        ...
        public function is_column_value($column, $value) {
            $table = $this->TableName;
            if ($table == null) {
                CreateLogRami("Can t Find Table Name in is_column_value");
            }
